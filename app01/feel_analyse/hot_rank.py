...
# 读取数据
list1 = []
res = cursor.execute("select hot_title from app01_hot_rank")
...
list3 = []
res = cursor.execute("select hot_num from app01_hot_rank")
for i in cursor.fetchall():
    for j in i:
        list3.append(j)
print(list3)
...
sql = 'insert into app01_hot_rank_result(jiji,zhongxing,xiaoji,num,key_s,title, hot_num,href) values(%s,%s,%s,%s,%s,%s,%s,%s)'
# # 读取数据
for i in range(len(nums)):
    data = (jiji,zhongxing,xiaoji,nums[1][i],nums[0][i],list1[i],list3[i],list4[i])
