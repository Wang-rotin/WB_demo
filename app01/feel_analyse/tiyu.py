...
# 读取数据
list1 = []
res = cursor.execute("select hot_title from app01_tiyu_rank")
...
list3 = []
res = cursor.execute("select hot_num from app01_tiyu_rank")
for i in cursor.fetchall():
    for j in i:
        list3.append(j)
print(list3)
...
sql = 'insert into app01_tiyu_result(jiji,zhongxing,xiaoji,num,key_s,title, hot_num,href) values(%s,%s,%s,%s,%s,%s,%s,%s)'
# # 读取数据
