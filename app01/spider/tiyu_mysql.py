...
# 创建 CSV 文件用于写入数据
with open('tiyu.csv', 'a', encoding='utf-8', newline='') as f:
    # 定义 CSV 的列名
...
try:
    ...
    # 删除数据库表格中的所有数据
    cursor.execute("DELETE FROM app01_tiyu_rank")

    # 插入数据到数据库
    for i in range(len(rank)):
        cursor.execute(
            "INSERT INTO app01_tiyu_rank (`rank`, hot_href, hot_title, hot_num) VALUES (%s, %s, %s, %s)",
